...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `
            <img class="game-img" src="${currentGame.img}" alt="${currentGame.name} game image" />
            <h2 class="game-name">${currentGame.name}</h2>
            <p class="game-description">${currentGame.description}</p>
            <p><strong>Backers:</strong> ${currentGame.backers.toLocaleString()}</p>
        `;
...
A total of $${GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString()} has been raised for ${GAMES_JSON.length} games.
